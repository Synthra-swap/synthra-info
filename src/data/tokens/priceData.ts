...
export async function fetchTokenPriceData(
  ...
  try {
    ...
    let allFound = false
    let attempts = 0
    const maxAttempts = 5 // Limita il numero di tentativi
    
    console.log('Starting price data fetch with params:', { address, startTimestamp, interval })
    
    while (!allFound && attempts < maxAttempts) {
      attempts++
      console.log(`Price data fetch attempt ${attempts}/${maxAttempts}, skip: ${skip}`)
      ...
      if (!loading) {
        skip += 100
        console.log(`Price data response: ${priceData?.tokenHourDatas?.length || 0} records, errors:`, errors)
        
        if ((priceData && priceData.tokenHourDatas.length < 100) || errors || attempts >= maxAttempts) {
          allFound = true
        }
        if (priceData) {
    ...
    console.log(`Price data fetch completed. Total records: ${data.length}`)
