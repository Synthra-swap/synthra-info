...
export default function TokenPage() {
  ...
  // Debug: log the price data to see what's happening
  useEffect(() => {
    console.log('Price data for token:', formattedAddress, priceData)
    console.log('Price data length:', priceData?.length)
    if (priceData && priceData.length > 0) {
      console.log('First price entry:', priceData[0])
      console.log('Last price entry:', priceData[priceData.length - 1])
    }
  }, [priceData, formattedAddress])
  ...
  const adjustedToCurrent = useMemo(() => {
    if (priceData && tokenData && priceData.length > 0) {
      ...
      return adjusted
    } else if (tokenData && tokenData.priceUSD) {
      // Se non ci sono dati storici, crea un punto dati con il prezzo corrente
      const currentTimestamp = Math.floor(Date.now() / 1000)
      return [
        {
          time: currentTimestamp - 3600, // 1 ora fa
